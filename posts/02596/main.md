...
<h2>名前解決</h2>
<ul>
 	<li>IPアドレスとホスト名（www.yahoo.co.jp等）はどう変換されているか？
<ul>
 	<li>DNSサーバ
<ul>
 	<li>IPアドレスとホストを管理
<ul>
 	<li>ネットワーク上に存在</li>
 	<li>ブラウザやpingでホスト名が指定されるとDNSにと言わせが行く
<ul>
 	<li>問い合わせ先のDNSサーバのIPアドレスは通常/etc/resolv.confに書く（DHCPを使っていると勝手に書かれている）
<ul>
 	...
 	<li>/etc/hosts
<ul>
 	<li>静的にIPアドレスとホストを対応付けたい時はここに書く</li>
</ul>
</li>
</ul>
</li>
