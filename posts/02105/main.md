...
<ul>
 	<li>もし最適状態価値関数が存在するなら、ある状態[latex]\\forall s \\in \\mathcal{S}-\\mathcal{S}_\\text{f}[/latex]の値[latex]V^*(s)[/latex]は、次の値を最大にする行動[latex]a \\in \\mathcal{A}[/latex]を選んだ時の、次の値になる。</li>
 	<li>値: 行動[latex]a[/latex]を選ぶと、[latex]s \\in \\mathcal{S}-\\mathcal{S}_\\text{f}[/latex]から、ある確率に従って次の状態[latex]s'\\in \\mathcal{S} [/latex]に遷移するが、その先の値[latex]V^*(s')[/latex]と、その状態遷移に対する評価[latex]\\mathcal{R}_{ss'}^a[/latex]を足したものの期待値を取った値</li>
</ul>
</li>
</ul>
