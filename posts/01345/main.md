...
<ul>
 	<li>意味: [latex]\\boldsymbol{x}_{t-1}[/latex]で[latex]\\boldsymbol{u}_t[/latex]を選択したときの、遷移後の状態がしたがう確率密度関数[latex]p(\\boldsymbol{x})[/latex]に従って、[latex]\\boldsymbol{x}_t[/latex]が決まる</li>
</ul>
</li>
</ul>
</li>
 	<li></li>
 	<li>[latex]bel(\\boldsymbol{x}_t) = \\int_\\mathcal{X} p(\\boldsymbol{x}_t | \\boldsymbol{x}_{t-1},\\boldsymbol{u}_t)d\\boldsymbol{x}[/latex]</li>
</ul>
