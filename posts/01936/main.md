...
<ul>
 	<li>[latex]\\boldsymbol{x}^{(i)}_t \\sim p(\\boldsymbol{x} | \\boldsymbol{x}_{t-1}, \\boldsymbol{u}_t) \\qquad (i=1,2,\\dots,N)[/latex]
<ul>
...
<ul>
 	<li>[latex]ｗ^{(i)}_t \\quad  \\texttt{*=} \\quad \\ell(\\boldsymbol{x}^{(i)}_t | \\boldsymbol{z}_t)  \\qquad (i=1,2,\\dots,N)[/latex]
<ul>
 	<li>もとの重みに尤度をかける</li>
</ul>
</li>
 	...
 	<li>重みの合計を1に正規化
<ul>
 	<li>[latex]\\eta = \\sum_{i=1}^N ｗ^{(i)}_t [/latex]</li>
 	<li>[latex]ｗ^{(i)}_t \\quad \\texttt{/=} \\quad \\eta  \\qquad (i=1,2,\\dots,N)[/latex]</li>
</ul>
</li>
</ul>
