...
<span style="color: #00ffff;">左側と右側をそれぞれ[latex]L(x_{t-1},x),L(x)[/latex]とおきましょう</span>

[latex]= L(x_{t-1},x) + L(x)[/latex]
<h2><!--nextpage--></h2>
...
=\\eta\\int_{-\\infty}^{\\infty} e^{- \\frac{1}{2}L(x_{t-1},x)}dx_{t-1}\\int_{-\\infty}^{\\infty} e^{-\\frac{1}{2} L(x)}dx_{t-1}\\\\[/latex]

<span style="color: #ffff00;">左側の積分が定数になるので[latex]\\eta[/latex]に入れてしまう</span>
