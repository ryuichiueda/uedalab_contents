...
<h2>状態遷移確率</h2>
<ul>
 	<li>[latex]p(\\boldsymbol{x}_t | \\boldsymbol{x}_{t-1},  \\boldsymbol{u}_t ) \\approx \\eta \\exp\\left\\{ -\\frac{1}{2} [\\boldsymbol{x}_t -f(\\boldsymbol{x}_{t-1},\\boldsymbol{u}_t) -G_t(\\boldsymbol{x}_{t-1} - \\boldsymbol{\\hat{x}}_{t-1})  ]^T  R_t^{-1}[\\boldsymbol{x}_t -f(\\boldsymbol{x}_{t-1},\\boldsymbol{u}_t) -G_t(\\boldsymbol{x}_{t-1} - \\boldsymbol{\\hat{x}}_{t-1})  ]  \\right\\}[/latex]</li>
</ul>
