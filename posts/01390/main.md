...
<ul>
 	...
 	<li>[latex]bel_t[/latex]を以下から導出してみましょう
<ul>
...
[latex]bel_t(x) = \\eta e^{-\\frac{(x - \\hat{x_{t-1}} - \\Delta_t)^2}{2(\\sigma_{t-1}^2 + \\delta_t^2)}} = \\mathcal{N}(\\hat{x_{t-1}} + \\Delta_t,\\sigma_{t-1}^2 + \\delta_t^2)[/latex]
<ul>
