...
<h2>計算</h2>
<ul>
 	<li>[latex]bel(x) = \\eta \\exp{-\\frac{(x-\\hat{x})^2}{2\\sigma^2}}e^{-\\frac{(z-x)^2}{2\\zeta^2}} \\\\
=\\eta \\exp\\left\\{-\\frac{1}{2\\delta^2}x^2 -\\frac{1}{2\\sigma^2}x^2-\\frac{\\hat{x}}{\\delta^2}x -\\frac{z}{\\zeta^2}x \\right\\}  [/latex]</li>
</ul>
