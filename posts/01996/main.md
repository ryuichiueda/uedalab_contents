# ロボットシステム学2016第6/7回
01996 <h1 style="font-size: 250%;">ロボットシステム学</h1>
<h2>第6/7回</h2>
上田 隆一
...
<ul>
 	<li>「Peripheral specification」をクリック</li>
</ul>
</li>
...
<h2>レジスタアドレスのリマップ</h2>
<ul>
 	<li>GPIOのレジスタを配列にマッピング
<ul>
 	<li>マッピング: アドレスとアドレスを対応付けること</li>
 	<li>0x3f200000: GPIOのレジスタの最初のアドレス
<ul>
 	<li>bcm2835のpdfには書いてない</li>
 	<li>このアドレスの後のレジスタの配置は同じ（Peripheral specification90,91ページ）</li>
</ul>
</li>
 	<li>0xA0: 必要なアドレスの範囲（91ページを読んで設定）</li>
</ul>
</li>
</ul>
<pre><span style="color: #ffffff;">...</span>
<span style="color: #ffffff;"><span style="color: #ffff00;">#include &lt;linux/io.h&gt;　　　　　　　　</span>　 //ヘッダファイルを追加
...
<span style="color: #ffff00;">static volatile u32 *gpio_base = NULL;</span> //アドレスをマッピングするための配列をグローバルで定義
...
static int __init init_mod(void)
{
 int retval;

<span style="color: #ffff00;"> gpio_base = ioremap_nocache(0x3f200000, 0xA0); //追加</span>
...</span></pre>
<!--nextpage-->
<h2>GPIOピンを出力にする</h2>
<ul>
 	<li>ピンを出力にするレジスタに1を書き込む
<ul>
 	<li>ほかのレジスタの値はそのままにしないといけない</li>
</ul>
</li>
 	<li>Peripheral specificationのp. 90-92あたり
<ul>
 	<li>GPIO25の機能はGPFSEL2にある</li>
 	<li>GPFSEL2の17,16,15番目のビットを001に設定</li>
</ul>
</li>
</ul>
<pre><span style="color: #ffffff;">static int __init init_mod(void)</span>
<span style="color: #ffffff;">{</span>
<span style="color: #ffffff;">    int retval;</span>

<span style="color: #ffffff;">    gpio_base = ioremap_nocache(0x3f200000, 0xA0); //0x3f..:base address, 0xA0: region to map</span>

<span style="color: #ffff00;">    const u32 led = 25;</span>
<span style="color: #ffff00;">    const u32 index = led/10;//GPFSEL2</span>
<span style="color: #ffff00;">    const u32 shift = (led%10)*3;//15bit</span>
<span style="color: #ffff00;">    const u32 mask = ~(0x7 &lt;&lt; shift);//11111111111111000111111111111111</span>
<span style="color: #ffff00;">    gpio_base[index] = (gpio_base[index] &amp; mask) | (0x1 &lt;&lt; shift);//001: output flag</span>
<span style="color: #ffff00;">    //11111111111111001111111111111111</span>
<span style="color: #ffffff;">...</span></pre>
<!--nextpage-->
<h2>ON/OFFの書き込み</h2>
<ul>
 	<li>p. 90, 95</li>
 	<li>ON: 「GPFSET0」のGPIO25に対応するところに1を書き込む</li>
 	<li>OFF: 「GPCLR0」のGPIO25に対応するところに1を書き込む</li>
</ul>
<pre><span style="color: #ffffff;">static ssize_t led_write(struct file* filp, const char* buf, size_t count, loff_t* pos)</span>
<span style="color: #ffffff;">{</span>
<span style="color: #ffffff;"> char c;</span>
<span style="color: #ffffff;"> if(copy_from_user(&amp;c,buf,sizeof(char)))</span>
<span style="color: #ffffff;"> return -EFAULT;</span>

<span style="color: #ffff00;"> if(c == '0')</span>
<span style="color: #ffff00;"> gpio_base[10] = 1 &lt;&lt; 25;</span>
<span style="color: #ffff00;"> else if(c == '1')</span>
<span style="color: #ffff00;"> gpio_base[7] = 1 &lt;&lt; 25;</span>
...
<h2>補足</h2>
<ul>
 	<li>「ただLEDを光らせるだけでこんなに苦労しなければならないのか」と質問をいただいたので・・・</li>
 	<li>そんなことはない
<ul>
 	<li>/sys/class/gpioの操作</li>
 	<li>WiringPi</li>
</ul>
</li>
 	<li>デバドラを書いた意味
<ul>
 	<li>機械からユーザランドまでの仕組みの一部を理解した</li>
 	<li>デバイスドライバで利用できる他の機能の学習の機会を得た
<ul>
 	<li>タイマー</li>
 	<li>メモリをもっと使った処理を含むデバイスドライバ</li>
</ul>
</li>
</ul>
</li>
