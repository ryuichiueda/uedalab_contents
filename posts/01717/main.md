...
[latex]\\boldsymbol{z}[/latex]という観測値を得る確率の密度</li>
 	<li>[latex]\\ell(\\boldsymbol{x}|\\boldsymbol{z})[/latex]（尤度）: 観測値[latex]\\boldsymbol{z}[/latex]を得た場合に、[latex]\\boldsymbol{x}[/latex]が真の状態でありそうだという指標
<ul>
 	<li>[latex]p(\\boldsymbol{z}|\\boldsymbol{x})[/latex]: 状態[latex]\\boldsymbol{x}[/latex]と基本的には同じもので、因果をひっくり返したもの。</li>
