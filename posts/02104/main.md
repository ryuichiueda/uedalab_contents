...
<ul>
 	<li><span style="color: #ff0000;">[latex]V^* (s) = \\text{argmax}_a \\sum_{s' \\in \\mathcal{S}} P_{ss'}^a \\left[ V^*(s') + R_{ss'}^a \\right] [/latex]</span>
<ul>
 	<li>もし最適状態価値関数が存在するなら、ある状態[latex]\\forall s \\in \\mathcal{S}[/latex]の値[latex]V^*(s)[/latex]は、</li>
</ul>
</li>
</ul>
