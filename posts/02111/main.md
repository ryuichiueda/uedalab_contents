...
<h2>価値の計算（価値反復）</h2>
<ul>
 	<li>ある状態[latex]s[/latex]の[latex]V^*(s)[/latex]だけ分からない。あとの状態の[latex]V^*[/latex]の値は分かるという、都合の良い状況を考えてみましょう。</li>
 	<li>価値の計算式はベルマン方程式そのものとなる
<ul>
 	<li>[latex]V^* (s) = \\text{argmax}_a \\sum_{s' \\in \\mathcal{S}} \\mathcal{P}_{ss'}^a \\left[ V^*(s') + \\mathcal{R}_{ss'}^a \\right] [/latex]</li>
</ul>
</li>
 	<li>この計算を[latex]\\forall s \\in \\mathcal{S} - \\mathcal{S}_\\text{f}[/latex]に対して何度も計算していく
<ul>
 	<li>（状態を適切に設定すると）（遷移数という意味で）<span style="color: #ff0000;">終端状態に近い状態から[latex]V^* (s)[/latex]の値が収束していく</span></li>
</ul>
</li>
</ul>
