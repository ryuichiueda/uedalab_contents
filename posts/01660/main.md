...
<h2>状態方程式の書き換え</h2>
<ul>
 	<li>[latex]G_t = \\frac{\\partial f(\\boldsymbol{x},\\boldsymbol{u})}{\\partial\\boldsymbol{x}}\\big|_{\\boldsymbol{\\hat{x}}_{t-1},\\boldsymbol{u}_t}[/latex]とする
<ul>
...
<ul>
 	<li>[latex]\\boldsymbol{x}_t = f(\\boldsymbol{\\hat{x}}_{t-1},\\boldsymbol{u}_t) + G_t(\\boldsymbol{x}_{t-1} - \\boldsymbol{\\hat{x}}_{t-1}) +\\boldsymbol{\\varepsilon}_t[/latex]</li>
</ul>
</li>
</ul>
<h2><!--nextpage--></h2>
<h2>状態遷移確率</h2>
<ul>
 	<li>[latex]p(\\boldsymbol{x} | \\boldsymbol{x}_{t-1},  \\boldsymbol{u}_t ) \\approx \\eta e^{ -\\frac{1}{2} \\left[\\boldsymbol{x} -f(\\boldsymbol{\\hat{x}}_{t-1},\\boldsymbol{u}_t) -G_t(\\boldsymbol{x}_{t-1} - \\boldsymbol{\\hat{x}}_{t-1})  \\right]^T  R_t^{-1}\\left[\\boldsymbol{x} - f(\\boldsymbol{\\hat{x}}_{t-1},\\boldsymbol{u}_t) -G_t(\\boldsymbol{x}_{t-1} - \\boldsymbol{\\hat{x}}_{t-1}) \\right]  }[/latex]
<ul>
...
<ul>
 	...
 	<li>平均: [latex]\\boldsymbol{\\hat{x}}_{t-1}[/latex]</li>
 	<li>共分散: [latex]\\hat\\Sigma_{t-1}[/latex]</li>
</ul>
</li>
 	<li>移動後の[latex]bel_t[/latex]は
<ul>
 	<li>平均: [latex]f(\\boldsymbol{\\hat{x}}_{t-1},\\boldsymbol{u}_t) + G_t(\\boldsymbol{x}_{t-1} - \\boldsymbol{\\hat{x}}_{t-1}) [/latex]</li>
 	<li>共分散: [latex]G_t\\hat\\Sigma_{t-1}G_t^T + R_t[/latex]</li>
</ul>
</li>
</ul>
